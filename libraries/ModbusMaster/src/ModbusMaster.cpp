...
@ingroup setup
*/
ModbusMaster::ModbusMaster(bool balance)
{
  _idle = 0;
  _preTransmission = 0;
  _postTransmission = 0;
...
uint8_t ModbusMaster::ModbusMasterTransaction(uint8_t u8MBFunction)
{
  ...
  _sf.clearFrame();
  _sf.addBytes(u8ModbusADU,u8ModbusADUSize);
  ...
  _serial->write(_sf.pGetFrame(),_sf.getFrameSize());
  ...
  while (u8BytesLeft && !u8MBStatus)
  {
...
#endif
      if(_rf.receiveByte(_serial->read())){
        size_t len = _rf.getFrameSize();
        if(len>sizeof(u8ModbusADU)){
          len=sizeof(u8ModbusADU);
        }
        u8ModbusADUSize = len;
        u8BytesLeft = 0;
        memcpy(u8ModbusADU,_rf.pGetFrame(),len);
      }
    ...
    if(0==_u8MBSlave){ // Won't get a response for broadcast messages, so time-out immediately
